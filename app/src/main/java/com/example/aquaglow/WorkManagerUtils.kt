...
object WorkManagerUtils {
    ...
    /**
     * Schedules daily step count reset at midnight
     */
    fun scheduleStepCountReset(context: Context) {
        val workManager = WorkManager.getInstance(context)
        val STEP_RESET_WORK_NAME = "step_reset"

        // Cancel existing step reset work
        workManager.cancelUniqueWork(STEP_RESET_WORK_NAME)
        ...
        // Create one-time work request
        val stepResetWork = OneTimeWorkRequestBuilder<StepResetWorker>()
            .setInitialDelay(delay, TimeUnit.MILLISECONDS)
            .setConstraints(constraints)
            .build()
